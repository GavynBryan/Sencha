...
std::optional<BodySpawnRequest> ChoosePawnSpawn(
    ...
    const CompiledGameSettings* settings, Logger& log)
{
...
void PreparePawn(World& world, EntityId participant, EntityId, Logger& log)
{
    ...
    if (peer == kNetAuthorityPeer)
        log.Info("FpsGame: spawned a pawn for the player at this machine (pawn prefab)");
    else
        log.Info("FpsGame: spawned a pawn for peer {} (pawn prefab)", peer);
}
