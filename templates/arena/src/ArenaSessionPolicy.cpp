...
void RegisterArenaDataTypes(DataAssetTypeRegistry& types,
                               DataSchemaRegistry& schemas)
{
    RegisterArenaSettingsData(types, schemas);
}

void UnregisterArenaDataTypes(DataAssetTypeRegistry& types,
                                 DataSchemaRegistry& schemas)
{
    UnregisterArenaSettingsData(types, schemas);
}

ArenaSessionPolicy::ArenaSessionPolicy(Engine& engine, Logger& log)
    : Host(engine)
    , Log(log)
{
}

ArenaSessionPolicy::~ArenaSessionPolicy() = default;
...
void ArenaSessionPolicy::RegisterSystems(SystemRegisterContext& ctx)
{
...
RuntimeAssets& ArenaSessionPolicy::Assets()
{
...
DataAssetCacheHandle ArenaSessionPolicy::AcquireDataAsset(std::string_view path)
{
    AssetLease lease = Assets().Assets.LoadLease(path, AssetType::Data);
    if (!lease.IsValid())
    {
        Log.Warn("ArenaGame: '{}' did not load; running without it", path);
        return {};
    }
...
const CompiledArenaSettings* ArenaSessionPolicy::GameSettings()
{
    if (!GameSettingsAsset.IsValid())
        GameSettingsAsset = AcquireDataAsset(kGameSettingsPath);
    if (!GameSettingsAsset.IsValid())
        return nullptr;
    const CompiledArenaSettings* settings =
        Assets().DataAssets.TryGet<CompiledArenaSettings>(
            GameSettingsAsset.GetToken(), "game.settings");
    if (settings == nullptr)
        Log.Warn("ArenaGame: '{}' is not a game.settings", kGameSettingsPath);
    return settings;
}
