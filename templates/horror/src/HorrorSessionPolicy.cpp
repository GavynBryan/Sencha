...
void RegisterHorrorDataTypes(DataAssetTypeRegistry& types,
                               DataSchemaRegistry& schemas)
{
    RegisterHorrorSettingsData(types, schemas);
}

void UnregisterHorrorDataTypes(DataAssetTypeRegistry& types,
                                 DataSchemaRegistry& schemas)
{
    UnregisterHorrorSettingsData(types, schemas);
}

HorrorSessionPolicy::HorrorSessionPolicy(Engine& engine, Logger& log)
    : Host(engine)
    , Log(log)
{
}

HorrorSessionPolicy::~HorrorSessionPolicy() = default;

void HorrorSessionPolicy::Open()
{
    Engine& engine = Host;
...
void HorrorSessionPolicy::Close()
{
...
void HorrorSessionPolicy::RegisterSystems(SystemRegisterContext& ctx)
{
...
RuntimeAssets& HorrorSessionPolicy::Assets()
{
...
DataAssetCacheHandle HorrorSessionPolicy::AcquireDataAsset(std::string_view path)
{
    AssetLease lease = Assets().Assets.LoadLease(path, AssetType::Data);
    if (!lease.IsValid())
    {
        Log.Warn("HorrorGame: '{}' did not load; running without it", path);
        return {};
    }
...
const CompiledHorrorSettings* HorrorSessionPolicy::GameSettings()
{
    if (!GameSettingsAsset.IsValid())
        GameSettingsAsset = AcquireDataAsset(kGameSettingsPath);
    if (!GameSettingsAsset.IsValid())
        return nullptr;
    const CompiledHorrorSettings* settings =
        Assets().DataAssets.TryGet<CompiledHorrorSettings>(
            GameSettingsAsset.GetToken(), "game.settings");
    if (settings == nullptr)
        Log.Warn("HorrorGame: '{}' is not a game.settings", kGameSettingsPath);
    return settings;
}
...
void HorrorSessionPolicy::SetupInputMapping()
{
    World& world = Host.World().Entities();
    ...
    {
        Log.Error("HorrorGame: no input profile; the game has no controls");
        return;
    }
    ...
    if (actions == nullptr)
    {
        Log.Error("HorrorGame: input profile did not bind: {}",
                  DescribeBindErrors(bindings.Status(profile)));
        return;
    }

    HorrorInputActions& ids = world.HasResource<HorrorInputActions>()
        ? world.GetResource<HorrorInputActions>()
        : world.AddResource<HorrorInputActions>();
