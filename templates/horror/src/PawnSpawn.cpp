...
{
    if (!world.IsRegistered<HorrorStart>())
        return std::nullopt;
    ...
    for (EntityId entity : world.GetAliveEntities())
    {
        ...
        if (!world.HasComponent<HorrorStart>(entity))
            continue;
...
void PreparePawn(World& world, EntityId participant, EntityId, Logger& log)
{
    ...
    if (peer == kNetAuthorityPeer)
        log.Info("HorrorGame: spawned a pawn for the player at this machine (pawn prefab)");
    else
        log.Info("HorrorGame: spawned a pawn for peer {} (pawn prefab)", peer);
}
...
void SessionPlayerSystem::FollowLocalControl(World& world)
{
    ...
    if (Owner->Prediction().Predicts(subject))
        Log->Info("HorrorGame: predicting this player's own pawn");
}
