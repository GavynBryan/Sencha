...
| Lifetime | Component traits retain/release through World resources (at the time, one bridge resource per component; now the World's `AssetStoreTable`); refcount-zero frees immediately; runtime Vulkan destruction goes through the deletion queue. |
