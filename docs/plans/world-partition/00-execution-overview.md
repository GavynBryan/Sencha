...
| `11-zone-runtime-model.md` | (proposed design) | The zone runtime model: zones stay the flat residency atom, compiled children carry `SourceZone` place identity, regions carry demand configuration (`JoinsSpatialDemand` joins the doc 10 shape fields), radius and graph demand compose, transitions are promoted contacts plus authored logical links, containment is a layered lookup, dynamic-entity limits stated. Owner review before anything starts. |
| `12-spatial-compilation.md` | (proposed design) | The spatial compilation design: optional residency subdivision on a world-aligned grid with stable child ids, coverage versus content bounds, content re-bucketing rules, scoped evidence kernels shared with future nav/query/map compilers, bespoke labeling, contact extraction, transition promotion (gate, eligibility, annotation; never geometry alone), gate association, the runtime containment artifact, fixtures and stages. Owner review before anything starts. |
