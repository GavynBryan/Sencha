...
- No build-time codegen (UHT-style). Second toolchain stage, hurts legibility.
  **Reversed** (2026-09). The hand-written `TypeSchema<T>` this plan settled on
  restated the struct's members, their initializers, and the component's
  identity a second time, and every later fact (replication, removability,
  editor visual) grew another specialization beside it. Component metadata is
  now generated by `sencha-component-codegen` from annotations on the struct
  (`ecs/ComponentAnnotations.h`) into a `ComponentDefinition<T>` companion
  header; `TypeSchema<T>` and the other declarative specializations project
  from it. Behaviour (`ComponentTraits`, `ComponentStorageTraits`,
  `SceneFieldCodec`) stays handwritten. See docs/core-systems-map.md, "Scene
  Serialization" and "Adding New Things".
