...
core_files() {
    { find "$INC" "$SRC" -type f \( -name '*.h' -o -name '*.cpp' \) 2>/dev/null; } \
        | grep -vE '(ScriptRuntime|ScriptLink|WorldScriptHost|ScriptBehaviorSystem|ScriptSystemTick|ScriptObserverDispatch|ScriptAbilitySystem|ScriptSceneLink|ScriptComponents|ScriptComponentSerializer)\.' \
        | grep -vE '(ScriptCook)\.'
}
