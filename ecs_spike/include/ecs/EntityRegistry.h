...
{
    uint32_t ArchetypeId = UINT32_MAX; // index into World::Archetypes
...
// EntityRecord: one slot in the entity table.
struct EntityRecord
{
...
// EntityRegistry: owns the generational entity id table.
// Generation lives here only — storage reads EntityIndex, not EntityId.
// Generation checks happen at World API boundaries (TryGet, etc.).
...
public:
    ...
    // Set location by raw index (used after an entity has been created
    // but before its generation is validated — called only from World internals).
    ...
    {
        size_t alive = 0;
        for (const auto& r : Records) alive += r.Alive ? 1 : 0;
        return alive;
    }
