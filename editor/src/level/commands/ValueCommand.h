...
{
public:
    ValueCommand(T before, T after, std::function<void(const T&)> apply, LevelDocument& document)
...
[[nodiscard]] inline std::unique_ptr<ICommand> MakeMoveCommand(
    EntityId entity, Transform3f before, Transform3f after, LevelScene& scene, LevelDocument& document)
{
...
[[nodiscard]] inline std::unique_ptr<ICommand> MakeEditBrushMeshCommand(
    EntityId entity, BrushMesh before, BrushMesh after, LevelScene& scene, LevelDocument& document)
{
