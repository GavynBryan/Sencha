...
// (right-hand rule, so a positive drag angle is a positive rotation).
void RingBasis(int part, Vec3d& u, Vec3d& v)
{
    switch (part)
    {
    case 1: u = Vec3d(0.0f, 1.0f, 0.0f); v = Vec3d(0.0f, 0.0f, 1.0f); break; // X
    case 2: u = Vec3d(0.0f, 0.0f, 1.0f); v = Vec3d(1.0f, 0.0f, 0.0f); break; // Y
    default: u = Vec3d(1.0f, 0.0f, 0.0f); v = Vec3d(0.0f, 1.0f, 0.0f); break; // Z
    }
}
    ...
    ModifierFlags /*modifiers*/) const
{
    ...
    const Vec3d axis = AxisDirection(part);
    ...
    std::unique_ptr<IRotateApply> apply = (kind == MeshElementKind::Object)
