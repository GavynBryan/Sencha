                                    ...
                                    ManipulatorVisual& out) const
{
    ...
    const Vec4 boxColor(0.55f, 0.6f, 0.7f, 1.0f);
    const Vec4 handleColor(1.0f, 1.0f, 1.0f, 1.0f);
    ...
    // Handles: screen-constant squares in the view plane.
    const ViewportProjection projection(viewport);
    for (const Handle& h : BuildHandles(r.Min, r.Max, r.UAxis, r.VAxis, r.WAxis))
    {
        const float half = projection.WorldSizeForPixels(h.Center, kHandlePixels) * 0.5f;
        ...
        out.Lines.push_back({ .A = a, .B = b, .Color = handleColor });
        out.Lines.push_back({ .A = b, .B = cc, .Color = handleColor });
        out.Lines.push_back({ .A = cc, .B = d, .Color = handleColor });
        out.Lines.push_back({ .A = d, .B = a, .Color = handleColor });
    }
}
