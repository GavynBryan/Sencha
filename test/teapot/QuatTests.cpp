...
TEST(Quat, Inverse)
{
	Quatf q = Quatf::FromAxisAngle(Vec3(0.0f, 0.0f, 1.0f), Pi / 3.0f);
...
{
	Quatf q = Quatf::FromAxisAngle(Vec3(1.0f, 0.0f, 0.0f), Pi / 2.0f);
...
{
	Quatf rotateX = Quatf::FromAxisAngle(Vec3(1.0f, 0.0f, 0.0f), Pi / 2.0f);
	Quatf rotateZ = Quatf::FromAxisAngle(Vec3(0.0f, 0.0f, 1.0f), Pi / 2.0f);
	Quatf composed = rotateZ * rotateX;

	Vec3 result = composed.RotateVector(Vec3(0.0f, 1.0f, 0.0f));

	// rotateX first: +Y -> +Z, then rotateZ leaves +Z unchanged.
	ExpectVecNear(result, Vec3(0.0f, 0.0f, 1.0f));
}

TEST(Quat, CompoundMultiplication)
{
	Quatf q = Quatf::FromAxisAngle(Vec3(1.0f, 0.0f, 0.0f), Pi / 2.0f);
	Quatf r = Quatf::FromAxisAngle(Vec3(0.0f, 0.0f, 1.0f), Pi / 2.0f);
...
{
	Quatf q = Quatf::FromAxisAngle(Vec3(0.0f, 0.0f, 1.0f), Pi / 2.0f);
...
{
	Quatf unitAxis = Quatf::FromAxisAngle(Vec3(0.0f, 0.0f, 1.0f), Pi / 2.0f);
	Quatf scaledAxis = Quatf::FromAxisAngle(Vec3(0.0f, 0.0f, 5.0f), Pi / 2.0f);
...
{
	Quatf q = Quatf::FromAxisAngle(Vec3(0.0f, 0.0f, 1.0f), Pi / 2.0f);
	Vec3 result = q.RotateVector(Vec3(1.0f, 0.0f, 0.0f));

	ExpectVecNear(result, Vec3(0.0f, 1.0f, 0.0f));
}

TEST(Quat, RotateVectorHandlesNonUnitQuaternion)
{
	Quatf q = Quatf::FromAxisAngle(Vec3(0.0f, 0.0f, 1.0f), Pi / 2.0f) * 3.0f;
	Vec3 result = q.RotateVector(Vec3(1.0f, 0.0f, 0.0f));
...
{
	EXPECT_TRUE(Quatf::Identity().ToMat3().NearlyEquals(Mat3::Identity()));
}
...
{
	Quatf x = Quatf::FromAxisAngle(Vec3(1.0f, 0.0f, 0.0f), Pi / 2.0f);
	Quatf y = Quatf::FromAxisAngle(Vec3(0.0f, 1.0f, 0.0f), Pi / 2.0f);
	Quatf z = Quatf::FromAxisAngle(Vec3(0.0f, 0.0f, 1.0f), Pi / 2.0f);

	EXPECT_TRUE(x.ToMat3().NearlyEquals(Mat3::MakeRotationX(Pi / 2.0f)));
	EXPECT_TRUE(y.ToMat3().NearlyEquals(Mat3::MakeRotationY(Pi / 2.0f)));
	EXPECT_TRUE(z.ToMat3().NearlyEquals(Mat3::MakeRotationZ(Pi / 2.0f)));
}
...
TEST(Quat, ToMat4EmbedsRotation)
{
	Quatf q = Quatf::FromAxisAngle(Vec3(0.0f, 0.0f, 1.0f), Pi / 2.0f);
	Mat4 m = q.ToMat4();
	Vec3 result = m.TransformVector(Vec3(1.0f, 0.0f, 0.0f));

	ExpectVecNear(result, q.RotateVector(Vec3(1.0f, 0.0f, 0.0f)));
...
{
	Quatf unit = Quatf::FromAxisAngle(Vec3(0.0f, 0.0f, 1.0f), Pi / 2.0f);
