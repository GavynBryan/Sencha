...
{
	auto m = Mat3d::Identity();
	auto s = Mat3d::MakeScale(2.0f, 3.0f, 4.0f);
	m *= s;
	EXPECT_FLOAT_EQ(m[0][0], 2.0f);
...
{
	auto m = Mat3d::MakeScale(2.0f, 3.0f);
	EXPECT_FLOAT_EQ(m[0][0], 2.0f);
	EXPECT_FLOAT_EQ(m[1][1], 3.0f);
	EXPECT_FLOAT_EQ(m[2][2], 1.0f);
}
...
{
	auto m = Mat3d::MakeScale(2.0f, 3.0f, 4.0f);
	EXPECT_FLOAT_EQ(m[0][0], 2.0f);
	EXPECT_FLOAT_EQ(m[1][1], 3.0f);
	EXPECT_FLOAT_EQ(m[2][2], 4.0f);
}
...
{
	constexpr auto zero = Mat3d::Zero();
	constexpr auto id   = Mat3d::Identity();
...
{
	Vec3d t(0.0f, 0.0f, 0.0f);
	Vec3d r(0.0f, 0.0f, 0.0f);
	Vec3d s(1.0f, 1.0f, 1.0f);
...
{
	Vec3d t(1.0f, 2.0f, 3.0f);
	Vec3d r(0.0f, 0.0f, 0.0f);
	Vec3d s(2.0f, 2.0f, 2.0f);
	auto m = Mat4::MakeTRS(t, r, s);

	// Transform a point: should scale then translate
	Vec3d p(1.0f, 0.0f, 0.0f);
	Vec3d result = m.TransformPoint(p);
...
{
	constexpr float pi = 3.14159265358979323846f;
	Vec3d t(3.0f, -7.0f, 12.0f);
	Vec3d r(pi / 6.0f, pi / 4.0f, pi / 3.0f);
	Vec3d s(2.0f, 2.0f, 2.0f);
	auto m = Mat4::MakeTRS(t, r, s);
	auto inv = m.AffineInverse();
	auto product = m * inv;
