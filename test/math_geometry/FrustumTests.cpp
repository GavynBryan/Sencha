...
static Mat4 MakeTestVP()
{
	...
	Mat4 view = Mat4::MakeLookAt(Vec3d(0.0f, 0.0f, 0.0f), Vec3d(0.0f, 0.0f, -1.0f), Vec3d(0.0f, 1.0f, 0.0f));
...
TEST(Frustum, ContainsPointInsideFrustum)
{
	...
	EXPECT_TRUE(frustum.ContainsPoint(Vec3d(0.0f, 0.0f, -5.0f)));
	EXPECT_TRUE(frustum.ContainsPoint(Vec3d(0.0f, 0.0f, -50.0f)));
}
...
TEST(Frustum, DoesNotContainPointBehindCamera)
{
	...
	EXPECT_FALSE(frustum.ContainsPoint(Vec3d(0.0f, 0.0f, 5.0f)));
}
...
TEST(Frustum, DoesNotContainPointBeyondFar)
{
	...
	EXPECT_FALSE(frustum.ContainsPoint(Vec3d(0.0f, 0.0f, -200.0f)));
}
...
TEST(Frustum, DoesNotContainPointFarToTheSide)
{
	...
	EXPECT_FALSE(frustum.ContainsPoint(Vec3d(100.0f, 0.0f, -5.0f)));
}
...
TEST(Frustum, IntersectsSphereInside)
{
	...
	Spheref sphere(Vec3d(0.0f, 0.0f, -10.0f), 1.0f);
...
TEST(Frustum, IntersectsSphereCrossingBoundary)
{
	...
	Spheref sphere(Vec3d(0.0f, 0.0f, 0.5f), 5.0f);
	EXPECT_TRUE(frustum.IntersectsSphere(sphere));
}
...
TEST(Frustum, IntersectsAabbInside)
{
	...
	Aabb3f box(Vec3d(-1.0f, -1.0f, -11.0f), Vec3d(1.0f, 1.0f, -9.0f));
...
TEST(Frustum, DoesNotIntersectAabbBehindCamera)
{
	...
	Aabb3f box(Vec3d(-1.0f, -1.0f, 5.0f), Vec3d(1.0f, 1.0f, 10.0f));
...
TEST(Frustum, DoesNotIntersectAabbBeyondFar)
{
	...
	Aabb3f box(Vec3d(-1.0f, -1.0f, -200.0f), Vec3d(1.0f, 1.0f, -150.0f));
	EXPECT_FALSE(frustum.IntersectsAabb(box));
}
...
TEST(Frustum, IntersectsAabbCrossingNearPlane)
{
	...
	Aabb3f box(Vec3d(-0.5f, -0.5f, -0.5f), Vec3d(0.5f, 0.5f, 0.5f));
	EXPECT_TRUE(frustum.IntersectsAabb(box));
}
...
TEST(Frustum, OrthographicFrustumContainsExpectedPoints)
{
	...
	EXPECT_TRUE(frustum.ContainsPoint(Vec3d(0.0f, 0.0f, -50.0f)));
	EXPECT_TRUE(frustum.ContainsPoint(Vec3d(9.0f, 9.0f, -1.0f)));
	EXPECT_FALSE(frustum.ContainsPoint(Vec3d(11.0f, 0.0f, -50.0f)));
	EXPECT_FALSE(frustum.ContainsPoint(Vec3d(0.0f, 0.0f, -101.0f)));
}
