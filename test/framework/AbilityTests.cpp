...
TEST_F(AbilityFixture, RequireTagGatesActivation)
{
    ...
    EXPECT_FALSE(TryActivateAbility(world, hero, readyAbility, abilities, tags, effects, attrs)); // missing State.Ready
    Tags(hero).Grant(ready);
    EXPECT_TRUE(TryActivateAbility(world, hero, readyAbility, abilities, tags, effects, attrs));
}
...
TEST_F(AbilityFixture, BlockTagPreventsActivation)
{
    ...
    EXPECT_FALSE(TryActivateAbility(world, hero, strike, abilities, tags, effects, attrs));
}

TEST_F(AbilityFixture, CostIsPaidAndUnaffordableActivationFails)
{
    EntityId hero = MakeActor(50.0f, { strike });
    EXPECT_TRUE(TryActivateAbility(world, hero, strike, abilities, tags, effects, attrs));
    EXPECT_FLOAT_EQ(Attrs(hero).GetBase(stamina), 30.0f);
    EXPECT_TRUE(TryActivateAbility(world, hero, strike, abilities, tags, effects, attrs));
    EXPECT_FLOAT_EQ(Attrs(hero).GetBase(stamina), 10.0f);
    EXPECT_FALSE(TryActivateAbility(world, hero, strike, abilities, tags, effects, attrs)); // 10 < 20
...
TEST_F(AbilityFixture, CooldownTagBlocksReactivationUntilExpiry)
{
    ...
    EXPECT_TRUE(TryActivateAbility(world, hero, dash, abilities, tags, effects, attrs));
    EXPECT_TRUE(Tags(hero).HasExact(cooldownDash));
    EXPECT_FALSE(TryActivateAbility(world, hero, dash, abilities, tags, effects, attrs)); // on cooldown
    EXPECT_FLOAT_EQ(Attrs(hero).GetBase(stamina), 80.0f); // no second charge

    TickEffects(world, 3.0f, effects, attrs); // cooldown expires
...
TEST_F(AbilityFixture, MutuallyBlockingAbilitiesCannotCoexist)
{
    ...
    EXPECT_TRUE(TryActivateAbility(world, hero, abilityA, abilities, tags, effects, attrs));
    EXPECT_TRUE(Tags(hero).HasExact(stateA));
    EXPECT_FALSE(TryActivateAbility(world, hero, abilityB, abilities, tags, effects, attrs)); // blocked by State.A

    EntityId other = MakeActor(100.0f, { abilityA, abilityB });
    EXPECT_TRUE(TryActivateAbility(world, other, abilityB, abilities, tags, effects, attrs));
    EXPECT_FALSE(TryActivateAbility(world, other, abilityA, abilities, tags, effects, attrs)); // blocked by State.B
}

TEST_F(AbilityFixture, UngrantedAbilityCannotActivate)
{
    EntityId hero = MakeActor(100.0f, { strike }); // not granted Dash
    EXPECT_FALSE(TryActivateAbility(world, hero, dash, abilities, tags, effects, attrs));
}

TEST_F(AbilityFixture, IntentQueueDrains)
{
    EntityId hero = MakeActor(100.0f, { abilityA });
    queue.Pending.push_back({ hero, abilityA });
    ProcessAbilityActivations(world, queue, abilities, tags, effects, attrs);
    EXPECT_TRUE(Tags(hero).HasExact(stateA));
