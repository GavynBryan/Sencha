...
TEST(AffordanceHandleMath, AabbFacesEnforceThicknessAndSideConstraint)
{
    ...
    const auto clampedToSide = AffordanceHandleMath::ResizeAabbFace(
        original, 2, true, 8.0, {}, 0.0f);
    ASSERT_TRUE(clampedToSide.has_value());
    EXPECT_FLOAT_EQ(clampedToSide->Max.Z, 0.0f);
...
TEST(AffordanceHandleMath, RectangleEdgesResizeSymmetricallyWithMinimum)
{
    const auto width = AffordanceHandleMath::ResizeRectangleEdge({ 1, 2 }, 0, -3.5);
    ASSERT_TRUE(width.has_value());
    EXPECT_FLOAT_EQ(width->X, 3.5f);
    EXPECT_FLOAT_EQ(width->Y, 2.0f);

    const auto minimum = AffordanceHandleMath::ResizeRectangleEdge({ 1, 2 }, 1, 0.0);
    ASSERT_TRUE(minimum.has_value());
    EXPECT_GT(minimum->Y, 0.0f);
}
