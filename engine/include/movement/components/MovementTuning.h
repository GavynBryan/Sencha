...
// The handle wrapper is addressed by asset-field code at the member's own
// offset, the way a mesh handle is: ownership and the editors both copy handle
// bytes without naming the wrapper. That only works while the wrapper is
// exactly its handle.
...
template <>
struct ComponentTraits<MovementTuningSource> : SchemaAssetOwnership<MovementTuningSource> {};
