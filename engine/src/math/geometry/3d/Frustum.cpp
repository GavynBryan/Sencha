...
Frustum Frustum::FromViewProjection(const Mat4& vp)
{
	...
	f.Planes[Left] = Plane(
		Vec3d(vp[3][0] + vp[0][0], vp[3][1] + vp[0][1], vp[3][2] + vp[0][2]),
	...
	f.Planes[Bottom] = Plane(
		Vec3d(vp[3][0] + vp[1][0], vp[3][1] + vp[1][1], vp[3][2] + vp[1][2]),
	...
	f.Planes[Near] = Plane(
		Vec3d(vp[3][0] + vp[2][0], vp[3][1] + vp[2][1], vp[3][2] + vp[2][2]),
...
bool Frustum::ContainsPoint(const Vec3d& point) const
{
...
bool Frustum::IntersectsAabb(const Aabb3d& box) const
{
	for (int i = 0; i < PlaneCount; ++i)
	{
		Vec3d pVertex;
